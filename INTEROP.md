...
How **hookcraft** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `hookcraft` lives in the **Sanctions - ownership - finance** cluster.
...
| from | relation | to |
|---|---|---|
| `hookcraft` | JSON findings compose with | [`corpmap`](https://github.com/cognis-digital/corpmap), [`cryptotrace`](https://github.com/cognis-digital/cryptotrace), [`maritimeint`](https://github.com/cognis-digital/maritimeint) |
| `hookcraft` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `hookcraft` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `hookcraft` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the sanctions - ownership - finance cluster.** `hookcraft` output feeds the next tool:
```bash
hookcraft ... --format json > out.json          # this tool's findings (see `hookcraft --help`)
...
hookcraft ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
hookcraft ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
hookcraft ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `hookcraft`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `hookcraft` slots
